...
public class DatabaseTest {
    ...
    @Когда("меняем название храма (\\d+) в менеджер$")
    public void передаем_в_менеджер(String arg1) throws Throwable {
        current = baseManager.update(arg1);
        Assert.assertNotNull(current);
    }

    @Тогда("^менеджер возвращает \"([^\"]*)\"$")
    public void менеджер_возвращает(arg1) throws Throwable {
